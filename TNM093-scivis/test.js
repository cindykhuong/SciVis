...
for (let i = 0; i < cutoff * 4; i += 4) { //allt blir svart
  // Set RGBA all to 0
  data[i] = 0;
  data[i + 1] = 0;
  data[i + 2] = 0;
...
const opacity = Number(document.getElementById("transfer-opacity").value);
const redwidth  = Number(document.getElementById("width-red").value);
const greenwidth = Number(document.getElementById("width-green").value);
const bluewidth = Number(document.getElementById("width-blue").value);
// const threshold = Number(document.getElementById("transfer-threshold").value) / 100;

const redPosition  = Number(document.getElementById("pos-red").value);
const greenPosition = Number(document.getElementById("pos-green").value);
const bluePosition = Number(document.getElementById("pos-blue").value);



for (let i = redPosition*4; i < redPosition*4 + redwidth*4; i += 4) {
      data[i] = 255; // Red
      data[i + 1] = 0; // Green
      data[i + 2] = 0; // Blue
      data[i + 3] = 0; // Alpha
}
for (let i = greenPosition*4; i < greenPosition*4 + greenwidth*4; i += 4) {
      data[i] = 0; // Red
      data[i + 1] = 255; // Green
      data[i + 2] = 0; // Blue
      data[i + 3] = 0; // Alpha
}
for (let i = bluePosition*4; i < bluePosition*4 + bluewidth*4; i += 4) {
      data[i] = 0; // Red
      data[i + 1] = 0; // Green
      data[i + 2] = 255; // Blue
      data[i + 3] = 0; // Alpha
...
//   data[i + 1] = i * green; //G
//   data[i + 2] = i * blue; //B
//   data[i + 3] = i * opacity; //A
// }

// for (let i = 0; i < 256 * 4; i += 4) {
//     // convert i into a value [0, 256] and set it 
//     // console.log(it)
//     const it = i * opacity / 4; //så vi får 256 för r,g,b & a
//     if (i/4 < threshold * 255) { //it/opacity
//       data[i] = 0; // Red
//       data[i + 1] = 0; // Green
//       data[i + 2] = 0; // Blue
//       data[i + 3] = 0; // Alpha 
...
//       data[i + 2] =  blue; // Blue
//       data[i + 3] = it; // Alpha(opacity) 
//     }
//   }

  //data är texturen som innehåller alla färger
  //256 pixlar i datan
  //opacity går mellan 0 pch 1
...
<body onload="main();">
  ...
  <p>
    ...
    <div style="position: absolute; top: 95; right: 500; width: 100px; text-align:center;">
      <!-- Transfer function -->
      <h1>Transfer function</h1>
      <div> Opacity: <input type="range" min="0" max="100" value="2" class="slider" id="transfer-opacity" oninput="triggerTransferFunctionUpdate()"></div>
      <div> Redwidth: <input type="range" min="1" max="100" value="100" class="slider" id="width-red" oninput="triggerTransferFunctionUpdate();"></div>
      <div> Greenwidth: <input type="range" min="1" max="100" value="100" class="slider" id="width-green" oninput="triggerTransferFunctionUpdate();"></div>
     <div> Bluewidth: <input type="range" min="1" max="100" value="100" class="slider" id="width-blue" oninput="triggerTransferFunctionUpdate();"></div>
     <!-- <div> Threshold: <input type="range" min="0" max="100" value="0" class="slider" id="transfer-threshold" oninput="triggerTransferFunctionUpdate();"></div>  -->
    </div>
  <p> 

  </p>
    <div style="position: absolute; top: 175; right: 300; width: 100px; text-align:center; ">
      <!-- Transfer function -->
    <div> Red Position: <input type="range" min="0" max="255" value="100" class="slider" id="pos-red" oninput="triggerTransferFunctionUpdate();"></div>
    <div> Green Position: <input type="range" min="0" max="255" value="100" class="slider" id="pos-green" oninput="triggerTransferFunctionUpdate();"></div>
    <div> Blue Position: <input type="range" min="0" max="255" value="100" class="slider" id="pos-blue" oninput="triggerTransferFunctionUpdate();"></div>
    </div>
  </p>

  <p> 
    <p style="position: absolute; top: 175; right: 620; width: 100px; text-align:center; ">
      <label for="colorpicker2">Color:</label>
  		<input type="color" id="colorpicker2" value="#ff0000" onchange="triggerTransferFunctionUpdate();">

      <label for="colorpicker3">Color:</label>
  		<input type="color" id="colorpicker3" value="#00ff00" onchange="triggerTransferFunctionUpdate();">

      <label for="colorpicker1">Color:</label>
      <input type="color" id="colorpicker1" value="#0000ff" onchange="triggerTransferFunctionUpdate();">
    </p>
  ...
  <!--
    Add some UI elements here to modify the transfer function.  As mentioned above, you need to call the function 
    'triggerTransferFunctionUpdate' to trigger and update of the GPU representation of the transfer function texture. 
     If you don't call that function, no change will be visible.
  -->
